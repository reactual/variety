...
var convertResults = function(interimResults, documentsCount) {
  ...
  for(var key in interimResults) {
    ...
    var parentGet = function(key) {
      if (key.split('.').length && key.split('.').length > 1) {
        var keyVal = key.split('.')[key.split('.').length - 2];
        if (keyVal === "$") {
          return key.split('.')[key.split('.').length - 3];
        } else {
          return keyVal;
        }
      } else {
    ...
    var keyFunction = function(key, keyTypex) {
